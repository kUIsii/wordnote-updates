...
| 释义编辑 | 详情页释义条目支持编辑文本，复制单词时同步复制释义记录 |
| 三种显示模式 | 正常顺序/仅当日/折叠三种模式循环切换，搜索在所有模式下正常工作 |
| 测验双向切换 | 测验中点"记得"可反悔为"不记得"，释义显示含蓝色标记 |
| 同分类重复检测 | 同分类重复单词检测：允许"意思相近的单词"重复，其他分类阻止重复 |
...
- 释义拆分和编辑
  - 修复复制单词到其他分类时不复制 WordMeaning 记录的 bug
  - 详情页释义条目新增"编辑"按钮，弹出对话框修改释义文本
  - 编辑后自动同步 word.meaning 字段
- 测验流程优化
  - 同时显示"记得"和"不记得"按钮
  - 点"记得"→显示释义（白色单词），"记得"按钮变为"不记得"可反悔
  - 点"不记得"→显示释义（红色单词），无反悔
  - 反悔后 forgetCount+1，记入不熟悉列表
- 三种显示模式
  - 平铺（正常顺序）→ 仅当日 → 折叠 三种模式循环切换
  - 仅当日模式：只显示当天添加的单词，无日期头
  - 搜索在所有模式下正常工作，搜索结束后自动恢复当前模式
- 同分类重复检测
  - "意思相近的单词"分类允许同名重复，仅提醒
  - 其他分类同名重复被阻止
  - 跨分类重复完全允许（无提醒）
  - 批量输入也支持重复检测
