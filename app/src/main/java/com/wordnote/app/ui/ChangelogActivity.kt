    ...
    private fun setupChangelog() {
        ...
        val changelog = listOf(
            VersionLog(
                "v2.20.5", "2026-06-06",
                listOf(
                    "复制到「意思相近的单词」后+按钮不显示：修复分类切换时序问题",
                    "释义拆分逻辑不稳定：修复详情页加载竞态条件，拆分按钮稳定显示",
                    "空分类下左右翻动失效：修复手势检测开关未打开的问题",
                    "「出现在其他分类」精确定位：点击后自动滚动到目标单词位置",
                    "追加单词自动拆分释义：追加时同步创建释义记录"
                )
            ),
            VersionLog(
                "v2.20.4", "2026-05-28",
            ...
            VersionLog(
                ...
                listOf(
                    ...
                    "测验流程优化：记得/不记得双向切换，记得后可反悔为不记得",
                    "三种显示模式：正常顺序/仅当日/折叠循环切换",
                    "同分类重复检测：允许「意思相近的单词」分类重复，其他分类阻止重复"
                )
            ),
            VersionLog(
                "v2.19.6", "2026-05-27",
            ...
            VersionLog(
                ...
                listOf(
                    ...
                    "分类切换优化：启动时预加载所有分类单词数据，滑动/点击切换分类时瞬间显示"
                )
            ),
            VersionLog(
                "v2.19.3", "2026-05-26",
